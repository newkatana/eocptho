...
<select id='filterText' class="form-select form-select-sm" onchange='filterText()'>
    <?php
            $sql_vac_sum = "SELECT * FROM eoc_allday order by alldate desc";
                            $query_vac_sum = mysqli_query($con,$sql_vac_sum);
                            $wdate;
                            while($row = mysqli_fetch_assoc($query_vac_sum)){
                                $wdate = $row['alldate']; ?>
                    <option value='<?php echo $row['alldate'];?>'><?php echo $row['alldate'];?></option>
    <?php } ?>
    <?php
            $sql_vac_sum2 = "SELECT * FROM eoc_allday order by alldate desc limit 1";
                            $query_vac_sum2 = mysqli_query($con,$sql_vac_sum2);
                            $wdate2;
                            while($row = mysqli_fetch_assoc($query_vac_sum2)){
                                $wdate2 = $row['alldate']; ?>
    <?php } ?>
</select>
</div>
<table id="table_format" class="table table-bordered table-hover table-sm">
                <thead class="text-center" style="background-color:#f2f2f2;">
                        <tr>
                            <th rowspan="2" style="min-width: 200px;">โรงพยาบาล</th>
                            <th colspan="6">SV</th>
                            <th colspan="6">AZ</th>
                            <th colspan="6">PZ</th>
                            <th colspan="6">SP</th>
                    ...
                    <tbody  id="myTable">
<?php 
$sql_vac_sum = "SELECT * FROM eoc_hospital_name_code order by hospital_code";
                $query_vac_sum = mysqli_query($con,$sql_vac_sum);
                $hoscode;
                // $hosname;
                while($row = mysqli_fetch_assoc($query_vac_sum)){
                    $hoscode = $row['hospital_code'];
                    $hosname = $row['ref_hospital_name']; ?>
                    <?php 
                            $sql_stock = "SELECT * 
                            FROM
                        ...
                        <tr class="content text-right">
                            <td class="text-left d-none"><?php echo $row['alldate'] ; ?></td>
                            <td class="text-left"><?php echo $hosname ; ?></td>
...
<script>
$("#filterText").val(<?php echo json_encode($wdate2);?>).change();
function filterText()
	{  
